...
public class WarInviteWidget extends Widget {
    ...
    public WarInviteWidget(int x, int y, int width, int height, String teamName, String teamTag, UUID teamUUID) {
    ...
    public void onClick(double mouseX, double mouseY) {
        ...
        if (mouseX >= startXAccept && mouseX <= startXAccept + 16 && mouseY >= startYAccept && mouseY <= startYAccept + 16) {
            Packets.sendToServer(new WarInviteResponsePacket(this.teamUUID, true));
        } else if (mouseX >= startXDeny && mouseX <= startXDeny + 16 && mouseY >= startYDeny && mouseY <= startYDeny + 16) {
            Packets.sendToServer(new WarInviteResponsePacket(this.teamUUID, false));
        }
    }
}
