...
public class InviteEntryWidget extends Widget {
    ...
    private final String teamName, teamTag;
    private final UUID teamUuid;

    public InviteEntryWidget(int x, int y, int width, int height, String teamName, String teamTag, UUID teamUuid) {
    ...
    public void onClick(double mouseX, double mouseY) {
        ...
        if (mouseX >= startXAccept && mouseX <= startXAccept + 16 && mouseY >= startYAccept && mouseY <= startYAccept + 16) {
            Packets.sendToServer(new AcceptInvitePacket(this.teamUuid));
        } else if (mouseX >= startXDeny && mouseX <= startXDeny + 16 && mouseY >= startYDeny && mouseY <= startYDeny + 16) {
            Packets.sendToServer(new DenyInvitePacket(this.teamUuid));
        }
    }
}